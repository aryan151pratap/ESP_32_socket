		...
		<div className={`h-full flex bg-${color}-800/90`}>
			...
			{ open &&
				<div className="w-[165px] h-full flex flex-col p-1 font-thin">
					...
					{new_wifi &&
						<div className={`mt-2 text-[16px] bg-${color}-700/50 overflow-hidden flex flex-col p-[1px]`}>
							<span>HOST</span>
							<input type="text" value={host.hostname} onChange={(e) => sethost(prevState => ({...prevState, hostname: e.target.value}))} className={`bg-${color}-700/50 mb-2 outline-none px-2 text-[14px]`} placeholder="Enter host" spellCheck={false}/>
							<span>PASSWORD</span>
							<input type="text" value={host.pass} onChange={(e) => sethost(prevState => ({...prevState, pass: e.target.value}))} className={`bg-${color}-700/50 outline-none px-2 text-[14px]`} placeholder="Enter password" spellCheck={false}/>
							<button className={`w-10 bg-${color}-700 text-green-500 p-1 flex justify-center mt-1`}
