				...
				<div className="w-[165px] h-full flex flex-col p-1 font-thin">
					...
					{new_wifi &&
						<div className={`mt-2 text-[16px] bg-${color}-700 overflow-hidden flex flex-col p-[1px]`}>
							<span>HOST</span>
							<input type="text" value={host.hostname} onChange={(e) => sethost(prevState => ({...prevState, hostname: e.target.value}))} className={`bg-${color}-800/50 mb-2 outline-none px-2 text-[14px]`} placeholder="Enter host" spellCheck={false}/>
							<span>PASSWORD</span>
							<input type="text" value={host.pass} onChange={(e) => sethost(prevState => ({...prevState, pass: e.target.value}))} className={`bg-${color}-800/50 outline-none px-2 text-[14px]`} placeholder="Enter password" spellCheck={false}/>
